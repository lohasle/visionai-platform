  ...
  <main class="operations-page">
    ...
    <section class="summary-grid">
      ...
      <article
        ...
        ><span>项目资产用量</span></article
      >
      <article
        ><small>外部实例</small><strong>{{ integrations.instances.length }}</strong
        ><span>{{ healthyInstances }} HEALTHY</span></article
      >
    </section>
    <el-tabs v-model="tab">
          ...
          <section class="panel"
            ...
            <el-table :data="resources.nodes"
              ><el-table-column prop="name" label="节点" /><el-table-column
                prop="gpuModel"
                label="GPU" /><el-table-column
                prop="gpuCount"
                label="数量"
                width="70" /><el-table-column prop="driverVersion" label="驱动" /><el-table-column
                prop="cudaVersion"
                label="CUDA" /><el-table-column prop="status" label="状态"
            /></el-table>
          </section>
          <section class="panel"
            ><div class="section-title"
      ...
      <el-tab-pane label="外部集成" name="integrations">
        <section class="panel">
          <el-table :data="integrations.instances"
            ...
            ><el-table-column label="操作"
              ><template #default="{ row }"
                ><el-button link type="primary" @click="test(row.id)">连接测试</el-button></template
              ></el-table-column
            ></el-table
          >
        </section>
        <section class="panel incidents"
          ...
          >
          <article
            v-for="incident in integrations.incidents"
            :key="incident.id"
            class="compact-card"
            ><div
              ><b>{{ incident.code }}</b
              ><span>{{ incident.message }}</span></div
            ><el-button v-if="incident.status !== 'RESOLVED'" @click="replay(incident.id)"
              >重放</el-button
            ></article
          >
        </section>
      ...
      <el-tab-pane label="系统审计" name="audit">
        <section class="panel"
          ><div class="section-title"
            ><div><h2>不可变审计日志</h2><p>按用户、项目、动作、资源、Trace 与时间追溯。</p></div
            ><a class="export-link" href="/admin-api/ai-platform/audit-events/export"
              >导出 CSV</a
            ></div
          >
          <el-table :data="audits" max-height="520"
            ><el-table-column prop="createTime" label="时间" width="180" /><el-table-column
              ...
              label="用户"
              width="75"
            /><el-table-column prop="projectId" label="项目" width="75" /><el-table-column
              prop="action"
              label="动作"
              min-width="210"
            /><el-table-column prop="resourceType" label="资源" width="150" /><el-table-column
              prop="resourceId"
              label="ID"
              width="75"
    ...
    <el-dialog v-model="integrationVisible" title="接入外部实例" width="560"
      ...
      /><el-form label-position="top" class="dialog-form"
        ...
        ><el-form-item label="Base URL"><el-input v-model="integrationForm.baseUrl" /></el-form-item
...
const loadAll = async () => {
  ;[resources.value, integrations.value, audits.value] = await Promise.all([
    getResourceOverview(),
    getIntegrations(),
    getAuditEvents()
  ])
...
.operations-page {
  ...
  background: radial-gradient(circle at 5% 0, rgb(37 99 235 / 8%), transparent 32%);
}
