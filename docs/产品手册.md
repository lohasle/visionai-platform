...
平台强制四眼原则：提交人不能审批自己的请求。租户可配置 1–10 个顺序审批步骤，每一步绑定系统管理中的 Reviewer、Approver、OPS 或 Project Owner 角色；多级步骤由不同人员完成。审批证据冻结当时的 Model Card、真实失败样本、评估门禁、数据/预训练权重/框架/模型制品四类许可、用途声明、风险和回滚方案，并生成覆盖制品、配置和目标环境的输入指纹。任一受控输入变化会将原审批标记为 `INVALIDATED`，阻止生产部署和模型导出。

模型导出必须填写用途。平台在返回制品清单前重新计算审批指纹，并记录下载人、用途、审批请求和制品数量；受控导入只接受平台对象存储 URI，并逐字节复算 SHA-256 和大小。
...
所有关键业务事件可以在审计列表查询并导出 CSV。同步故障支持重放、重新绑定、忽略和人工关闭，均保存处置原因与 Before/After。集成升级必须命中 `ALLOWED` 兼容规则；平台先保存配置快照和 SHA-256，再执行 Provider 内部健康检查，失败时自动恢复原配置，并保留差异、冒烟结果和回退原因。
